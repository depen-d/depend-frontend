...
export default function Tasks() {
  ...
  const createTask = (
    newTask: Omit<Task, "task_id" | "code" | "created_at" | "updated_at">
  ) => {
    axios
  ...
  const editTask = (task?: Partial<Task>) => {
    ...
    const { code, ...data } = task;

    axios
      .patch(`${baseURL}/task/${code}`, data)
      .then((response) => {
  ...
  const checkRelatedTasksConflict = async (task: Task) => {
    console.log(task);
    if (!task?.dependencies?.length) return [];

    const conflictingTasks = tasks.filter(
      (related) =>
        task.dependencies?.includes(related.code) &&
        (related.status === "OPEN" || related.status === "IN_PROGRESS")
    );
  ...
  const updateTaskStatus = (task: Task, newStatus: Task["status"]) => {
    editTask({ code: task.code, status: newStatus });
  };
  ...
  const moveTask = async (task: Task, newStatus: Task["status"]) => {
    const conflictingTasks = await checkRelatedTasksConflict(task);
