...
export default function Case({ useCase, onEdit, onDelete }: CaseProps) {
  ...
  const teamsProgress = useMemo(() => {
    return {
      ...
      [Teams.DESIGN]: calculateProgress(
        filteredTasks.filter((task) => task.team === Teams.DESIGN)
      ),
      [Teams.DEV]: calculateProgress(
        filteredTasks.filter((task) => task.team === Teams.DEV)
      ),
      [Teams.TEST]: calculateProgress(
        filteredTasks.filter((task) => task.team === Teams.TEST)
      ),
    };
  }, [filteredTasks]);
              ...
              <div className="flex justify-between text-sm">
                ...
                <span>
                  {(teamsProgress[Teams.REQ] +
                    teamsProgress[Teams.DESIGN] +
                    teamsProgress[Teams.DEV] +
                    teamsProgress[Teams.TEST]) /
                    4}
                  %
                </span>
              </div>
              <div className="flex gap-2">
                <Progress value={teamsProgress[Teams.REQ]} className="h-2" />
                <Progress value={teamsProgress[Teams.DESIGN]} className="h-2" />
                <Progress value={teamsProgress[Teams.DEV]} className="h-2" />
                <Progress value={teamsProgress[Teams.TEST]} className="h-2" />
              </div>
            </div>
